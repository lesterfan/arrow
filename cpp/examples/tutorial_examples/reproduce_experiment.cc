...
arrow::Status generate_field_data(std::shared_ptr<arrow::Array>& out, int64_t num_rows,
  ...
  if (field->type()->id() == arrow::Type::INT64) {
    ...
    std::string value = std::string(2, 'a');
...
arrow::Status repro() {
  ...
  const int64_t total_rows = 100'000;
  ...
  auto props = parquet::WriterProperties::Builder().max_row_group_length(chunk_size)->build();
  auto arrow_props = parquet::default_arrow_writer_properties();
  ARROW_ASSIGN_OR_RAISE(
      auto writer, parquet::arrow::FileWriter::Open(*schema, arrow::default_memory_pool(),
