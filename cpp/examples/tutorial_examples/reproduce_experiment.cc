...
arrow::Status parseRowGroupOfParquetFile(int row_group_num, const std::string& filename,
  ...
  std::cout << "Parsing " << filename << " to read row group " << row_group_num << ", column " << column
            << "\n";
  ...
  std::shared_ptr<parquet::arrow::RowGroupReader> row_group_reader = reader->RowGroup(row_group_num);
...
int main(int argc, char* argv[]) {
  ...
  arrow::Status st = parseRowGroupOfParquetFile(FLAGS_row_group, FLAGS_filename, FLAGS_column);
  if (!st.ok()) {
